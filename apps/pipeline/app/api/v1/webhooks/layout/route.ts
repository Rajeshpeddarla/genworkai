...
export async function POST(request: Request) {
  try {
    ...
    const { job_id, schema_version, status, pages_processed, pages_failed, extracted_data, processing_time_ms } = payload;
    ...
    const finalExtractedData = {
      ...
      assets: extracted_data.assets || [],
      metadata: {
        ...(extracted_data.metadata || { model: "unknown" }),
        debug: {
          usage: {
            pages_processed: pages_processed || realPageCount,
            pages_failed: pages_failed || 0,
            pages_retried: 0,
    ...
    if (planRes.rows.length > 0) {
      ...
      await client.query(`
        ...
        SET pages_extracted_this_month = pages_extracted_this_month + ($1 * 4)
        WHERE id = $2
    ...
    await client.query(`
      INSERT INTO request_logs (user_id, file_name, status, execution_time_ms, request_metadata)
      VALUES ($1, $2, $3, $4, $5)
    `, [userId, fileName, 'success', processing_time_ms, JSON.stringify({ pages: realPageCount, tokens: finalExtractedData.metadata.debug.usage })]);
