...
export default function SupportClient({ initialTickets, currentUserId }: { initialTickets: any[], currentUserId: string }) {
  ...
  return (
    <div className="h-screen bg-background text-foreground flex flex-col font-sans overflow-hidden">
      <nav className="px-6 py-4 border-b border-zinc-200 dark:border-white/10 flex justify-between items-center shrink-0">
        <Link href="/workspace" className="inline-flex items-center gap-2 text-zinc-500 dark:text-zinc-400 hover:text-foreground transition-colors text-sm font-medium">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/3 max-w-sm border-r border-zinc-200 dark:border-white/10 flex flex-col bg-card shrink-0">
            ...
            {initialTickets.length === 0 ? (
              ...
              initialTickets.map(ticket => (
                <div 
                  ...
                  className={`p-4 border-b border-zinc-200 dark:border-white/10 cursor-pointer transition-colors ${
                    activeTicket === ticket.id 
                      ? 'bg-black/5 dark:bg-white/5 border-l-4 border-l-emerald-500' 
                      : 'hover:bg-black/5 dark:hover:bg-white/5 border-l-4 border-l-transparent'
                  }`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-bold text-foreground text-sm truncate pr-2">{ticket.subject}</span>
                    <span className="text-xs text-zinc-500 dark:text-zinc-400 shrink-0">{new Date(ticket.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between items-center mt-2">
                    ...
                    <span className={`flex items-center gap-1 text-[11px] font-medium ${
                      ...
                      ticket.status === 'pending' ? 'text-orange-400' : 'text-zinc-500 dark:text-zinc-400'
                    }`}>
        ...
        <div className="flex-1 flex flex-col bg-background relative">
          {activeTicket === "new" ? (
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
              <div className="max-w-2xl mx-auto">
                <div className="mb-8">
                  ...
                  <p className="text-zinc-500 dark:text-zinc-400 text-sm">Please provide details about your issue so our team can assist you.</p>
                </div>
                
                <form onSubmit={handleCreateSubmit} className="space-y-5 bg-black/5 dark:bg-white/5 border border-zinc-200 dark:border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-background border border-zinc-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:border-emerald-500"
                      >
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-background border border-zinc-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:border-emerald-500 resize-none"
                  ...
                  <div className="pt-2 flex justify-end gap-3">
                    <button
                      ...
                      className="px-4 py-2 rounded-lg text-sm font-medium text-zinc-500 dark:text-zinc-400 hover:text-foreground transition-colors"
                    >
                      Cancel
                    </button>
                    <button
            ...
            <>
              {/* Chat Header */}
              <div className="px-6 py-4 border-b border-zinc-200 dark:border-white/10 shrink-0 flex justify-between items-start bg-card">
                <div>
                  <h2 className="text-lg font-bold text-foreground mb-1">{currentTicket.subject}</h2>
                  <div className="flex items-center gap-3 text-xs">
                    <span className="text-zinc-500 dark:text-zinc-400 capitalize">{currentTicket.type}</span>
                    <span className="text-zinc-700">•</span>
                    <span className={`font-medium ${
                      currentTicket.status === 'open' ? 'text-emerald-400' : 
                      currentTicket.status === 'pending' ? 'text-orange-400' : 'text-zinc-500 dark:text-zinc-400'
                    }`}>
              ...
              {/* Chat Messages */}
              <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-black/5 dark:bg-white/5">
                ...
                {currentTicket.messages?.map((msg: any, idx: number) => (
                  <div key={idx} className={`flex flex-col ${!msg.isAgent ? 'items-end' : 'items-start'}`}>
                    <div className="flex items-baseline gap-2 mb-1">
                      <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400">{msg.isAgent ? 'Support Agent' : 'You'}</span>
                    ...
                    <div className={`p-4 rounded-2xl max-w-[80%] text-sm leading-relaxed ${
                      !msg.isAgent 
                        ...
                        : 'bg-card border border-zinc-200 dark:border-white/10 text-foreground rounded-tl-sm shadow-sm'
                    }`}>
                      {msg.content}
                    </div>
                  </div>
              ...
              {currentTicket.status !== 'resolved' && (
                <div className="p-4 border-t border-zinc-200 dark:border-white/10 shrink-0 bg-black/5 dark:bg-white/5">
                  <div className="flex items-center gap-2 max-w-4xl mx-auto">
                    <button className="p-2 text-zinc-500 dark:text-zinc-400 hover:text-zinc-500 dark:text-zinc-400 transition-colors">
                    ...
                    <input 
                      ...
                      className="flex-1 bg-background border border-zinc-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-sm text-foreground focus:ring-2 focus:ring-emerald-500/50 outline-none"
                      onKeyDown={(e) => {
                    ...
                    <button 
                      onClick={handleReply}
                      disabled={!reply}
                      className="p-2.5 bg-emerald-500 text-white rounded-xl disabled:opacity-50 disabled:bg-zinc-200 dark:disabled:bg-zinc-800 hover:bg-emerald-600 transition-colors"
                    >
