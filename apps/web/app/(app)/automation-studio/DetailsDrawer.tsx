      ...
      {isOpen && (
        <>
          ...
          <motion.div 
            ...
            className="fixed right-0 top-0 bottom-0 w-[55%] min-w-[600px] bg-[#0f0f0f] border-l border-zinc-200 dark:border-zinc-800 z-50 flex flex-col shadow-2xl"
          >
            <div className="flex items-center justify-between p-6 border-b border-zinc-200 dark:border-zinc-800 shrink-0 bg-[#0a0a0a]">
              <div>
                <h2 className="text-2xl font-bold text-zinc-900 dark:text-white flex items-center gap-3">
                  ...
                  <span className={`px-2 py-0.5 rounded text-xs font-bold border capitalize ${
                    ...
                    'bg-gray-500/10 text-zinc-500 dark:text-zinc-500 dark:text-zinc-400 border-gray-500/20'
                  }`}>
                    {automation.status}
                  </span>
                </h2>
                <p className="text-sm text-zinc-500 dark:text-zinc-500 dark:text-zinc-400 mt-1">{automation.description || 'No description provided.'}</p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button 
                  onClick={onClose}
                  className="p-2 hover:bg-gray-800 rounded-lg text-zinc-500 dark:text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:text-white transition-colors ml-2"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col">
              {/* Overview Grid */}
              <div className="p-6 grid grid-cols-2 lg:grid-cols-4 gap-4 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#111]">
                <div>
                  <div className="text-xs text-gray-500 flex items-center gap-1 mb-1"><Database className="w-3 h-3" /> Database</div>
                  <div className="text-sm font-medium text-zinc-900 dark:text-white">{db?.databaseName || 'Unknown'}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 flex items-center gap-1 mb-1"><Clock4 className="w-3 h-3" /> Schedule</div>
                  <div className="text-sm font-medium text-zinc-900 dark:text-white capitalize">{automation.schedule || 'Manual'}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 flex items-center gap-1 mb-1"><PlayCircle className="w-3 h-3" /> Total Runs</div>
                  <div className="text-sm font-medium text-zinc-900 dark:text-white">{automation.totalRuns || 0}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 flex items-center gap-1 mb-1"><Clock4 className="w-3 h-3" /> Created</div>
                  <div className="text-sm font-medium text-zinc-900 dark:text-white">{new Date(automation.createdAt).toLocaleDateString()}</div>
                </div>
              </div>
              ...
              <div className="p-6 flex-1 flex flex-col min-h-0">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-zinc-900 dark:text-white text-lg">Execution History</h3>
                  <button onClick={fetchHistory} className="text-zinc-500 dark:text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:text-white transition-colors">
                ...
                <div className="space-y-6 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                  {history.map((log) => (
                    <div key={log.id} className="bg-black/5 dark:bg-white/5 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden flex flex-col">
                      <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-white dark:bg-[#111]">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-sm text-zinc-500 dark:text-zinc-500 dark:text-zinc-400 font-medium">
                      ...
                      {log.artifactContent && (
                        <div className="flex-1 flex flex-col">
                          <div className="px-4 py-2 bg-[#1a1a1a] border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between text-xs">
                            <span className="text-zinc-500 dark:text-zinc-500 dark:text-zinc-400 font-medium flex items-center gap-1.5"><FileText className="w-3.5 h-3.5" /> Generated Artifact</span>
                            <div className="flex items-center gap-3">
                              <button onClick={() => handleCopy(log.artifactContent)} className="text-zinc-500 dark:text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:text-white transition-colors flex items-center gap-1">
                          ...
                          <div className="p-6 bg-[#0a0a0a] text-zinc-600 dark:text-zinc-600 dark:text-zinc-300 prose prose-invert prose-sm max-w-none prose-headings:text-indigo-400 prose-a:text-indigo-400">
